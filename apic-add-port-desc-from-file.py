...
# Read port_name from "Lxx_list.txt"
leaf_file = leaf_name + '_list.txt'
print(leaf_file)
with open (leaf_file, 'r') as portnames:
#portnames = open (leaf_file, 'r')
	for port_name in portnames:
		port_name = port_name.strip()
		jsondata = '''
		{
		  "infraPortBlk": {
		    "attributes": {
		      "dn": "uni/infra/accportprof-%s/hports-%s-typ-range/portblk-block2",
		      "descr": "%s"
		    },
		    "children": []
		  }
		}
		''' % (leaf_name, port_name, port_name)
		r = s.post('https://{0}/api/mo/uni/infra/accportprof-{1}/hports-{2}-typ-range/portblk-block2.json'.format(apic,leaf_name,port_name), cookies=cookies, data=jsondata, verify=False)
		print(r)
		print(jsondata)
		print(r.status_code)
